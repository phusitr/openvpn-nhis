...
;Modern UI Configuration
  ...
  !define MUI_WELCOMEPAGE_TEXT "OpenVPN -  an Open Source VPN package by James Yonan.\r\n\r\nOpenVPN GUI v11.6 Icons - Created by Matthew Marable\r\n\r\nCustom Installer Create
d by Matthew Marable"
...
Section "OpenVPN Interactive Service" SecService
  ...
  SetOutPath "$INSTDIR\bin"
	...
	${If} ${RunningX64}
		File "${BIN}\x64\openvpnserv.exe"
		File "${BIN}\x64\openvpnserv2.exe"
  ${Else}
    File "${BIN}\x86\openvpnserv.exe"
    File "${BIN}\x86\openvpnserv2.exe"
  ${EndIf}
...
Function .onInit
  ...
  StrCmp $R0 "" goon
    ...
    Messagebox MB_YESNO "It seems the package ${PRODUCT_NAME} (OpenVPN GUI) is already installed.$\r$\nWe recommend you to uninstall it in the standard way before proceeding. C
ontinue installing?" IDYES goon
    Abort
...
m be killed now? If true, all existing connections will be closed." IDNO donerun
